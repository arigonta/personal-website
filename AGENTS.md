...
- Uppercase mono is reserved for chrome and true metadata: section numbers, dates, the card index, the ROLE label, and the eyebrow. Product types, tags, chips, capability labels, service labels, nav links, and footer facts read as normal sentence case sans.
- Sections and cards fade and rise once on scroll via an IntersectionObserver in `app/page.tsx`. The `reveal` class is only ever added by that script, so content stays visible without JavaScript, and the script exits early under `prefers-reduced-motion`.
- Generous spacing, restrained motion, and strong readability still govern everything.
